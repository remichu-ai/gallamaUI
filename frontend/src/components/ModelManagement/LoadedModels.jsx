...
const LoadedModels = () => {
    const { loadedModels, fetchLoadedModels, stopModelByPort } = useModelManagementStore();

    useEffect(() => {
        fetchLoadedModels();
        // Set up an interval to fetch loaded models every 30 seconds
        const interval = setInterval(fetchLoadedModels, 30000);
    ...
    return (
        <Box className={styles.container}>
            <div className={styles.headerContainer}>
                ...
                <button onClick={fetchLoadedModels} className={styles.refreshButton} aria-label="Refresh loaded models">
            ...
            <List spacing={3}>
                {Object.entries(loadedModels).map(([modelName, modelData]) => (
                    modelData.instances.map((instance, index) => (
                        <ListItem key={`${modelName}-${instance.port}`} className={styles.listItem}>
                            <Text>
